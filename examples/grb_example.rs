...
fn get_int_attr<const D: usize>(grb_model: *mut GRBmodel, attr: &'static [u8; D]) -> i32 {
    unsafe {
        ...
        let ret_code = GRBgetintattr(grb_model, attr as *const u8 as *const libc::c_char, ptr as *mut libc::c_int);
        if ret_code != 0 {
            panic!("failed!");
        }
...
fn get_dbl_attr<const D: usize>(grb_model: *mut GRBmodel, attr: &'static [u8; D]) -> f64 {
    unsafe {
        ...
        let ptr = Box::into_raw(ret);
        let ret_code = GRBgetdblattr(grb_model, attr as *const u8 as *const libc::c_char, ptr as *mut f64);
        if ret_code != 0 {
            panic!("failed!");
        }
...
fn main() {
    ...
    unsafe {
        let ret_code = GRBnewmodel(prob.grb_env, &mut prob.grb_model as *mut *mut GRBmodel, "prob".as_ptr() as *const libc::c_char, 0, 0 as *mut f64, 0 as *mut f64, 0 as *mut f64, 0 as *mut i8, 0 as *mut *mut i8);
        if ret_code != 0 {
    ...
    unsafe {
        for company in &prob.companies {
            ...
            let ret_code = GRBaddvar(prob.grb_model, 0, 0 as *mut libc::c_int, 0 as *mut f64, -company.profit, 0., 1., GRB_BINARY as libc::c_char, var_name.as_ptr() as *const libc::c_char);
            if ret_code != 0 {
    ...
    unsafe {
        ...
        let ret_code = GRBaddconstr(prob.grb_model, indexs.len() as libc::c_int, indexs.as_ptr() as *mut libc::c_int, vals.as_ptr() as *mut f64, GRB_GREATER_EQUAL as libc::c_char, prob.min_property, "".as_ptr() as *const libc::c_char);
        if ret_code != 0 {
    ...
    unsafe {
        ...
        let ret_code = GRBaddconstr(prob.grb_model, indexs.len() as libc::c_int, indexs.as_ptr() as *mut libc::c_int, vals.as_ptr() as *mut f64, GRB_LESS_EQUAL as libc::c_char, prob.max_libability, "".as_ptr() as *const libc::c_char);
        if ret_code != 0 {
    ...
    println!("time: {}", get_dbl_attr(prob.grb_model, GRB_DBL_ATTR_RUNTIME));
    println!("possible best obj: {}", get_dbl_attr(prob.grb_model, GRB_DBL_ATTR_OBJBOUND));
    ...
    unsafe {
        ...
        let ret_code = GRBgetdblattrarray(prob.grb_model, GRB_DBL_ATTR_X as *const u8 as *const libc::c_char, 0, prob.companies.len() as libc::c_int, solution.as_mut_ptr());
        if ret_code != 0 {
