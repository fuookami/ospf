...
mod custom_dimension_tests {
    use super::*;
    use crate::dimension::{CustomFundamentalDimension, DerivedQuantity, FundamentalDimension, FundamentalQuantityEnum};
    ...
    fn test_custom_dimension() {
        ...
        let mass_quantity = DerivedQuantity::from_base("mass".to_string(), FundamentalQuantityEnum::Mass);
        ...
        assert!(expected_symbol.contains("T"), "Expected symbol to contain 'T', got: {}", expected_symbol);
        assert!(expected_symbol.contains("M"), "Expected symbol to contain 'M', got: {}", expected_symbol);
        ...
        assert_eq!(quotient_symbol, "1", "Expected dimensionless, got: {}", quotient_symbol);
    }
    ...
    fn test_tome_dimension_calculations() {
        ...
        // 量纲计算: M ÷ (M/T) = M × T/M = T
        let kilogram_unit = create_custom_unit(
            "kilogram",
            "kg",
            mass_quantity.clone(),
            Scale::new(),
        ...
        assert!(result1.unit.dimension().symbol().contains("T"), 
            "Expected dimension symbol to contain 'T', got: {}", result1.unit.dimension().symbol());
        ...
        assert!(result2.unit.dimension().symbol().contains("M"), 
            "Expected dimension symbol to contain 'M', got: {}", result2.unit.dimension().symbol());
        ...
        let none_unit = create_custom_unit("dimensionless", "1", dimensionless_quantity, Scale::new());
        ...
        assert_ne!(three_tome.unit.dimension(), three_dimensionless.unit.dimension());
    ...
    fn test_zhang_and_ling_dimension_calculations() {
        ...
        assert!(result1.unit.dimension().symbol().contains("Z"), 
            "Expected dimension symbol to contain 'Z', got: {}", result1.unit.dimension().symbol());
        ...
        assert!(result2.unit.dimension().symbol().contains("L"), 
            "Expected dimension symbol to contain 'L', got: {}", result2.unit.dimension().symbol());
    ...
    fn test_custom_dimension_arithmetic() {
        ...
        assert!(product_symbol.contains("T"), "Expected dimension symbol to contain 'T', got: {}", product_symbol);
        ...
        assert!(result_symbol.contains("M"), "Expected dimension symbol to contain 'M', got: {}", result_symbol);
    }
