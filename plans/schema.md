...
- 线性、二次求解器统一接口，以及 Gurobi、SCIP、CPLEX、COPT、MindOPT、Hexaly 等插件。
...
2. 保证各插件无损映射原生状态；达到限制或被取消时，只要存在 incumbent 就必须保留解、目标、bound 和 gap。
...
9. 建立求解正确性 fixture、跨 backend 矩阵和 golden/replay 门禁。
...
- 再迁移 CPLEX、COPT、MindOPT、Hexaly 及其它实际可用插件。
- 占位或未完整实现的插件必须准确声明 capability，不得通过存在模块推断可用能力。

### Phase S2：统一取消与终止（P0）

- 建立 `SolveHandle`/`CancellationToken` 及 backend 原生中断适配。
- 打通同步、协程、`CompletableFuture`、组合求解和远程停止。
...
- 建立可用 backend 的矩阵测试；缺少许可证或原生库时显式标记 skipped，不冒充通过。
...
- [ ] `OSPF-SOL-009` 迁移其余可用插件；未实现能力必须声明为 unsupported，不能只写日志或在运行时静默忽略。

### 6.3 取消与资源释放

- [x] `OSPF-SOL-010` 定义每次求解独立的 `SolveHandle`/`CancellationToken`，并支持幂等取消和终止原因记录。
- [ ] `OSPF-SOL-011` 为各可用插件接入原生 terminate/interrupt/abort，保证协程和 `CompletableFuture` 取消能中断阻塞求解。
...
- [ ] `OSPF-SOL-016` 优先调用 backend 原生 IIS/Farkas；通用弹性过滤和删除过滤按实际保证标记为精确、不可约、启发式或未知。
...
- [ ] `OSPF-SOL-025` 建立跨 backend golden/replay 矩阵，校验状态、目标容差、约束残差、bound、IIS 和指纹一致性。
- [x] `OSPF-SOL-026` 提供纯函数式批量实验运行器、并发预算、确定性排序和结构化报告导出。
- [ ] `OSPF-SOL-027` 更新 core solver、各插件、framework combinatorial/remote 和 benchmark 双语文档，提供兼容迁移示例。
...
- [ ] 其余可用插件通过同一状态映射合同测试；不支持项由 capability 明确声明。
...
- [ ] 每次报告包含稳定 solver ID、backend/plugin/native 版本和实际生效配置；Gurobi 不同主版本可以区分。
...
插件集成测试应按 backend profile 分组。商业求解器许可证、原生库或远程服务不可用时，不重复运行完整构建；应从同一次日志中确认并在评测报告中记录环境前置条件。
