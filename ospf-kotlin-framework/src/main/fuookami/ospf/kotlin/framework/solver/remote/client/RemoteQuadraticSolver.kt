...
class RemoteQuadraticSolver(
    ...
    /**
     * 使用执行端口构造远程二次求解器。 / Construct remote quadratic solver with execution port.
     *
    ...
    */
    ...
    /**
     * 执行远程二次求解。 / Execute remote quadratic solve.
     *
    ...
    */
    ...
    private suspend fun SolveResult.toSolveReport(variableCount: Int): Ret<SolveReport<Flt64>> {
        ...
        val descriptor = SolverDescriptor(
            ...
            capabilities = SolverCapabilities(modelTypes = setOf(SolverModelType.QP, SolverModelType.QCP))
        )
        return Ok(
            SolveReport(
    ...
    */
     ...
     * 从对象存储读取序列化的求解结果。
     *
     * @return 序列化求解结果，如果不可用则返回 null / the serialized solution, or null if unavailable
    */
