/** SCIP 稳定身份到原生 artifact 的正向投影测试。 / SCIP stable identity to native artifact forward projection tests. */
...
/**
 * SCIP 线性求解器稳定身份原生投影集成测试。 / SCIP linear solver stable identity native projection integration test.
 */
class ScipLinearIdentityProjectionIT {
    /**
    ...
    fun stableIdentityIsProjectedToNativeNames() = runBlocking {
        ...
        val callBack = ScipSolverCallBack().configuration { _, _, variables, constraints ->
            capturedVariableNames.addAll(variables.map { it.name })
            capturedConstraintNames.addAll(constraints.map { it.name })
            ok
        }

        val result = ScipLinearSolver(callBack = callBack)(projectionModel(), null)
