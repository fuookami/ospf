    ...
    fn from(value: &ResponseDTO) -> Self {
        return crossbeam::scope(|s| {
            let future1 = s.spawn(|_| {
                ...
                for bin_data in &value.bins {
                    for item_data in &bin_data.items {
                        ...
                        if let Some(package_type) = &item_data.package_type {
                            package_types.insert(package_type);
                        }
                    }
                }
            ...
            let future2 = s.spawn(|_| {
                ...
                for bin_data in &value.bins {
                    if let Some(batch_no) = &bin_data.batch_no {
                        batch_nos.insert(batch_no.clone());
                    }
                    if let Some(bin_type) = &bin_data.type_code {
                        bin_types.insert(bin_type);
                    }
                }
