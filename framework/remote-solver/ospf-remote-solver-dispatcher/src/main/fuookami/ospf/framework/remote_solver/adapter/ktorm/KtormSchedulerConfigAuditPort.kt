...
class KtormSchedulerConfigAuditPort(
    ...
    override suspend fun list(limit: Int): List<SchedulerHotReloadAuditRecord> {
        ...
        val all = database
            ...
            .orderBy(auditTable.createdSeq.asc())
            .map { row ->
                ...
                connection.createStatement().use { statement ->
                    statement.execute(
                        ...
                        CREATE TABLE IF NOT EXISTS $resolvedAuditTableName (
                            created_seq BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                    ...
                    runCatching {
                        statement.execute(
                            "ALTER TABLE $resolvedAuditTableName " +
                                "ADD COLUMN created_seq BIGINT GENERATED BY DEFAULT AS IDENTITY"
                        )
                    }
                    statement.execute(
                        """
                        CREATE UNIQUE INDEX IF NOT EXISTS idx_${resolvedAuditTableName}_version
                        ON $resolvedAuditTableName (version)
                        """.trimIndent()
                    )
                    statement.execute(
                        """
                        CREATE UNIQUE INDEX IF NOT EXISTS idx_${resolvedSnapshotTableName}_version
                        ON $resolvedSnapshotTableName (version)
                        """.trimIndent()
                    )
                }
            }
    ...
    private fun decodeSnapshot(encoded: String): SchedulerRuntimeConfig? {
        ...
        return SchedulerRuntimeConfig(
            ...
            schedulerStarvationAgeMs = props["schedulerStarvationAgeMs"]?.toLongOrNull() ?: 30_000L,
            schedulerMigrationHysteresisRatio = props["schedulerMigrationHysteresisRatio"]?.toDoubleOrNull() ?: 0.15,
            schedulerMinSlicesBeforeMigration = props["schedulerMinSlicesBeforeMigration"]?.toIntOrNull() ?: 1,
            schedulerMigrationCostWeight = props["schedulerMigrationCostWeight"]?.toDoubleOrNull() ?: 0.25,
            schedulerCostWeight = props["schedulerCostWeight"]?.toDoubleOrNull() ?: 0.6,
            schedulerDeadlineRiskWeight = props["schedulerDeadlineRiskWeight"]?.toDoubleOrNull() ?: 0.3,
            schedulerQueueDelayWeight = props["schedulerQueueDelayWeight"]?.toDoubleOrNull() ?: 0.1,
            schedulerRoundRobinScoreTolerance = props["schedulerRoundRobinScoreTolerance"]?.toDoubleOrNull() ?: 0.05,
            schedulerWeightedRoundRobinEnabled = props["schedulerWeightedRoundRobinEnabled"]?.toBooleanStrictOrNull() ?: true,
            schedulerProgressWeight = props["schedulerProgressWeight"]?.toDoubleOrNull() ?: 0.25,
            schedulerProgressFastGapThreshold = props["schedulerProgressFastGapThreshold"]?.toDoubleOrNull() ?: 0.75,
            schedulerProgressCheapGapThreshold = props["schedulerProgressCheapGapThreshold"]?.toDoubleOrNull() ?: 0.20,
            schedulerProgressMinImprovement = props["schedulerProgressMinImprovement"]?.toDoubleOrNull() ?: 0.01,
            schedulerProgressNoImprovementSlices = props["schedulerProgressNoImprovementSlices"]?.toIntOrNull() ?: 1
        )
    }
