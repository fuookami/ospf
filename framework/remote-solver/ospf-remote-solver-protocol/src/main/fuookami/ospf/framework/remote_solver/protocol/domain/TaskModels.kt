...
data class TaskMeta(
    ...
    constructor(
        ...
        solutionLimit: Int? = null,
        estimatedVariableCount: Int? = null,
        estimatedConstraintCount: Int? = null,
        historicalRuntimeMs: Long? = null,
        metadata: Map<String, String> = emptyMap()
    ) : this(
        solverType = solverType?.let { SolverTypeName.of(it) },
        targetType = targetType?.let { TargetTypeName.of(it) },
        timeLimit = timeLimitMs?.toDuration(DurationUnit.MILLISECONDS),
        solutionLimit = solutionLimit,
        estimatedVariableCount = estimatedVariableCount,
        estimatedConstraintCount = estimatedConstraintCount,
        historicalRuntime = historicalRuntimeMs?.toDuration(DurationUnit.MILLISECONDS),
        metadata = metadata
    )

    /** 时间限制毫秒兼容别名 / Compatibility alias for time limit milliseconds */
    val timeLimitMs: Long? get() = timeLimit?.inWholeMilliseconds

    /** 历史运行时间毫秒兼容别名 / Compatibility alias for historical runtime milliseconds */
    val historicalRuntimeMs: Long? get() = historicalRuntime?.inWholeMilliseconds
}
...
data class SolverConfig(
    ...
    constructor(
        timeLimitMs: Long? = null,
        solutionLimit: Int? = null,
        mipGapTolerance: Double? = null,
        threads: Int? = null,
        solverParams: Map<String, String> = emptyMap()
    ) : this(
        timeLimit = timeLimitMs?.toDuration(DurationUnit.MILLISECONDS),
        solutionLimit = solutionLimit,
        mipGapTolerance = mipGapTolerance?.let { Flt64(it) },
        threads = threads,
...
data class SolvePayload(
    ...
    constructor(
        ...
        taskMeta: TaskMeta = TaskMeta(timeLimit = null),
