...
实现创建名称在 `name` 后追加 `_min` 的实数结果变量。精确模式创建名称追加 `_u_` 和索引的二值选择变量；尽可能从候选范围推导每个 Big-M，并注册上下界选择约束及 one-hot 等式。松弛模式省略选择变量和 Big-M 下界约束。
...
直接求值始终得到精确最小值。精确 solver 模式需要所有候选有效的 Big-M 范围；松弛模式只保证 y 对每个候选是上界。非负结果变量也可能与负的数学最小值冲突，因此注册前应比较候选域和变量域。
