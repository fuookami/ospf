...
Kotlin 创建 $u_i\in\{0,1\}$，用两条关系约束把每个受支持不等式连接到 $u_i$；`EQ` 则使用四约束零带模型。公开结果为
...
求解器侧结果仍是计数 $c$；Kotlin **不会** 创建上面直接求值公式所暗示的二值阈值结果。Rust 接收已经创建的指标，注册 $y-\sum_i u_i=0$；`with_amount_range` 再把上下界作为计数的硬约束。
