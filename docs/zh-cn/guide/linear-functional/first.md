...
Kotlin 先通过两约束正值指标模型把 $b_i\in\{0,1\}$ 连接到 $p_i>0$。预期的首个命中变量 $h_i\in\{0,1\}$ 可以写成
...
当前 Kotlin 注册还会额外传入单调约束 $h_i\le h_{i-1}$。该约束会与“较后位置首次命中”冲突，例如 $b=(0,1)$ 可能变为不可行。这是当前实现缺陷，不属于预期公式。

Rust 不创建 $b_i$，而是接收外部条件二值变量，创建首个激活选择变量，并把连续结果门控到被选候选多项式；没有激活项时，仅在启用 `zero_if_none` 后把结果固定为零。因此 Rust 返回被选值，不是 Kotlin 的首个索引。
