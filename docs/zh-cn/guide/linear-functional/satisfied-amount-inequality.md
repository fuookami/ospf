...
基础类为每个输入创建一个名称在 `name` 后追加 `_u_` 和索引的二值标志。存在 `amount` 时还创建二值 `name` + `_y`。当有限输入范围包含零时，solver 用两条带 epsilon 的 Big-M 约束将标志连接到扁平化关系；当零在范围外时，对支持的符号可以将标志固定为平凡满足/违反值。amount 范围由 amount 指标控制的两条放松约束实现。
