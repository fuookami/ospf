...
管理航班恢复调度系统中机组成员、排班和中转时间的领域模型，为航班串生成提供可行性和成本计算所需的机组数据。
...
> 机组上下文不直接定义优化变量，其数据作为航班串生成的输入参数。
...
> 机组上下文不直接定义优化约束，其数据通过 `ConnectionTimeCalculator` 和 `RuleChecker` 间接影响航班串生成的可行性判断。
