...
**$x_{b}^{(k)}$** ：第 $k$ 次迭代中航班串 $b$ 的决策变量，取值为 0 或 1，表示航班串 $b$ 是否被选中。
...
**$\text{passenger}_{i,cls}$** ：航班任务 $i$ 在舱位 $cls$ 上的旅客容量表达式。
...
**$x_{b}^{(k)}$** ：第 $k$ 次迭代中航班串 $b$ 的选择变量，无量纲量，取值范围为 $\{0, 1\}$，表示是否选择航班串 $b$ 执行恢复计划，$\forall b \in B^{(k)}$ 。

**$y_{i}$** ：任务 $i$ 的链接辅助变量，无量纲量，取值范围为 $\{0, 1\}$，表示任务 $i$ 是否被包含在选中的航班串中，$\forall i \in I$ 。

**$z_{a}$** ：飞机 $a$ 的车队平衡辅助变量，无量纲量，取值范围为 $\{0, 1\}$，表示飞机 $a$ 是否被使用，$\forall a \in A$ 。

### 2. 辅助变量

**$\text{link\_slack}_{l}$** ：链接 $l$ 的松弛变量，取值范围为 $[0, +\infty)$ ，用于惩罚不被任何选中航班串覆盖的链接，$\forall l \in L$ 。
...
**$B_{a}$** ：分配给飞机 $a$ 的航班串子集，$\forall a \in A$ 。

**$B_{i}$** ：包含任务 $i$ 的航班串子集，$\forall i \in I$ 。
