...
市场上有 $n$ 种资产 $S_{i}$ （ $i=1, 2, \cdots, n$ ）可以选择，使用数额为 $M$ 的资金进行一个时期的投资。这 $n$ 种资产在这一时期内购买 $S_{i}$ 的平均收益率为 $r_{i}$，风险损失率为 $q_{i}$，投资越分散，总的风险越小，总体风险可用投资的 $S_{i}$ 的投资风险加权投资金额得到。
...
设定一个投资组合方案，给定资金 $1000000$ 元，有选择地购买资产或者银行存款，在风险控制在 $2\%$ 以内的条件下，使得净收益最大。
