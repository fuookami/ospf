...
**$x_{ij}$** ：货物 $i$ 是否装载到位置 $j$，二元变量，取值范围为 $\{0, 1\}$，仅对可行对 $(i,j)\in IJ^{feas}$ 建模；状态固定的对会被固定为零或一。
...
**$u_{ij}$** ：货物 $i$ 在位置 $j$ 的调整变量，平衡三值变量，取值范围为 $\{-1,0,1\}$，仅对调整对 $(i,j)\in IJ^{adj}$ 建模；非调整对固定为零。
...
`stowage` 表达式是按状态处理的桥接：对需要装载的货物-位置对，
