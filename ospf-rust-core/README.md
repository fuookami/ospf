...
| 2 Structure | Solver-neutral `DeferredFunctionStructure` (symbol handle, helper columns, fixed Big-M, versioned fingerprint) | Implemented for ABS, NOT, AND, OR, MAX, MIN, SEMI, binaryzation, relation indicator, balance ternaryzation, sigmoid, IF, binary masking, sin, cos, polynomial masking, the MAX/MIN forwarding symbols (MinMax/MaxMin), slack range, IF-THEN, implication and IF-IN; every other function stays eager |
