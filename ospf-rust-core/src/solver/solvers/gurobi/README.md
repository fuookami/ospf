...
| `gurobi_pwl` | `functions-pwl-1` | `SinStructure`, `CosStructure` and `LogisticStructure` whose input is a single unit-coefficient monomial with a zero constant and whose point table has at least two finite, strictly increasing points | `add_genconstr_pwl` |
...
| `gurobi_in_values` | `functions-in-values-1` | `InValuesStructure` whose value set is non-empty, whose input is a single unit-coefficient monomial with a zero constant, and whose condition boxes are finite | `add_genconstr_indicator` (four per candidate value) + `add_genconstr_or` (aggregation) |
...
| `gurobi_conditional_value` | `functions-conditional-value-1` | `ConditionalThenStructure` (the conditional-value symbol: result = thenPoly when the condition holds, otherwise 0). A folded condition (constant branch coverage) falls back, as Gurobi has no general constraint that fixes a column to a constant | six equality indicators: the condition relation (boxed by the symbol's explicit finite `ConditionBounds`) plus branch equalities written on both inner columns |
| `gurobi_masking` | `functions-masking-1` | `MaskingStructure` (binary mask): the mask column must be binary, exclusive helpers, and the frozen M must satisfy `M ≥ |x|` (verified against the SDK input box; ±1e100 handled explicitly) | two equality indicators: `m=1 ⇒ y == x`, `m=0 ⇒ y == 0` (the eager `y ∓ M·m` rows are implied by the containment proof) |
