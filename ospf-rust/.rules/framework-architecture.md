...
本规范适用于 `ospf-rust-framework` 下以优化建模为核心的 framework 模块，包括但不限于 CSP1D、BPP3D、Gantt Scheduling 等领域框架。
...
1. 对外领域模型、配置和结果优先使用泛型数值抽象（如 `V: RealNumber` 等 trait bound）。
2. 宽度、长度、重量、面积、需求、产出、余料、容量等领域量应优先使用 `Quantity<V, U>`、`UnitTrait` 或明确的物理量类型表达。
3. `f64`、裸值只能作为 solver adapter、registration、extraction 或低层数学接口的实现细节。
...
2. 可变规则优先通过 strategy trait、context、pipeline、adapter、函数参数、trait 或领域 policy 注入。
