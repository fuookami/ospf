...
use ospf_rust_core::error::{CoreError, Result, SolverError, SolverModelingError};
...
use ospf_rust_core::solver::{
    SolutionPresence, SolveIssue, SolveReport, Solver, SolverExt, SolverOutput,
    linear_model_fingerprint, require_infeasibility_certificate,
    require_infeasibility_certificate_for_model, require_optimal_lp_certificate_for_linear_model,
...
use super::{CutSense, FrameworkSolveOptions};
...
pub(crate) fn solver_output_to_feasible(output: &SolverOutput) -> Result<FeasibleSolution> {
    ...
    if report.has_incumbent() {
        ...
        ) && report.solution.as_ref().is_some_and(|solution| {
            solution.objective_value.is_none() && solution.objective.is_none()
        }) {
            return Err(CoreError::Solver(SolverError::ContractViolation(
                "solver returned feasible status but missing objective value".to_owned(),
            )));
        }
        return FeasibleSolution::try_from_report(&report);
    }
    ...
    let err = match report.problem_status {
        ospf_rust_core::solver::ProblemStatus::Infeasible => SolverError::Infeasible,
        ospf_rust_core::solver::ProblemStatus::InfeasibleOrUnbounded => {
            SolverError::InfeasibleOrUnbounded
        }
        ospf_rust_core::solver::ProblemStatus::Unbounded => SolverError::Unbounded,
        _ => solver_error_from_report_without_incumbent(&report),
    };
    Err(CoreError::Solver(err))
}

fn solver_error_from_report_without_incumbent(report: &SolveReport<f64>) -> SolverError {
    match report.problem_status {
        ...
        ospf_rust_core::solver::ProblemStatus::Unbounded => SolverError::Unbounded,
        _ => match report.termination_reason {
            ospf_rust_core::solver::TerminationReason::TimeLimit => {
                SolverError::Timeout(std::time::Duration::ZERO)
            }
            ospf_rust_core::solver::TerminationReason::Cancelled => {
                SolverError::Cancelled("REPORT".to_owned())
            }
            ospf_rust_core::solver::TerminationReason::Interrupted => {
                SolverError::Interrupted("UserInterrupt".to_owned())
            }
            ospf_rust_core::solver::TerminationReason::NumericalFailure => {
                SolverError::NumericalError("solver report has no usable incumbent".to_owned())
            }
            ospf_rust_core::solver::TerminationReason::BackendFailure => {
                SolverError::SolveFailed(format!(
                    "solver returned no incumbent after backend failure: {:?}",
                    report.problem_status
                ))
            }
            _ => SolverError::NoSolution,
        },
    }
}
...
pub(crate) fn solution_vector_from_output(
    ...
    if !report.has_incumbent() {
        return Err(CoreError::Solver(SolverError::ContractViolation(format!(
            "{} did not produce an incumbent: {:?}",
            context, report.problem_status
        ))));
    }
    ...
    .map_err(|err| match err {
        CoreError::Solver(SolverError::NoSolution) => CoreError::Solver(
            SolverError::ContractViolation(format!("{} missing solution vector", context)),
        ),
        other => other,
    })
}
...
{
    let report = output.clone().try_into_solve_report()?;
    let model_fingerprint = linear_model_fingerprint(lp_model)?;
    let mut compatibility_report = report;
    // 兼容旧 SolverOutput 时重新绑定调用方模型指纹；新路径直接传 report，避免丢失来源。
    // Rebind the caller model fingerprint for the legacy facade; new paths pass the report directly.
    compatibility_report.fingerprints.model = Some(model_fingerprint);
    resolve_lp_dual_solution_from_report(solver, lp_model, &compatibility_report)
}

pub(crate) fn resolve_lp_dual_solution_from_report<S>(
    solver: &S,
    lp_model: &LinearTriadModel,
...
where
    S: Solver + Send + Sync,
{
    resolve_lp_dual_solution_from_report_with_options(solver, lp_model, report, None)
}

pub(crate) fn resolve_lp_dual_solution_from_report_with_options<S>(
    solver: &S,
    lp_model: &LinearTriadModel,
    report: &SolveReport<f64>,
    options: Option<&ospf_rust_core::solver::SolveOptions<'_>>,
) -> Result<Vec<f64>>
where
    S: Solver + Send + Sync,
{
    let model_fingerprint = linear_model_fingerprint(lp_model)?;
    require_optimal_lp_certificate_for_model(report, &model_fingerprint)?;
    if let Ok(certificate) = require_optimal_lp_certificate_for_linear_model(report, lp_model) {
        return Ok(certificate.dual.to_vec());
    }

    resolve_lp_dual_solution_by_model(solver, lp_model, options)
}

const CERTIFICATE_TOLERANCE: f64 = 1e-7;

fn approximately_equal_certificate(left: f64, right: f64) -> bool {
    let scale = left.abs().max(right.abs()).max(1.0);
    (left - right).abs() <= CERTIFICATE_TOLERANCE * scale
}

fn valid_model_dimensions(model: &LinearTriadModel) -> bool {
    model.c.len() == model.num_variables()
        && model.basic.lb.len() == model.num_variables()
        && model.basic.ub.len() == model.num_variables()
        && model.basic.b.len() == model.num_constraints()
        && model.basic.A.rows.len() == model.num_constraints()
}

fn valid_bound_pair(lower: f64, upper: f64) -> bool {
...
fn farkas_row_certificate_is_valid(lp_model: &LinearTriadModel, dual: &[f64]) -> bool {
    let expected_rows = lp_model.num_constraints();
    if dual.len() < expected_rows || !valid_model_dimensions(lp_model) {
        return false;
    }
    let rows = &dual[..expected_rows];
    if rows
        .iter()
        .any(|value| !value.is_finite() || *value < -CERTIFICATE_TOLERANCE)
    {
        return false;
    }
    if rows
        .iter()
        .all(|value| value.abs() <= CERTIFICATE_TOLERANCE)
    {
        return false;
    }

    let mut transposed_activity = vec![0.0; lp_model.num_variables()];
    for (row_index, row) in lp_model.basic.A.rows.iter().enumerate() {
        for (column, coefficient) in &row.entries {
            if *column >= transposed_activity.len() || !coefficient.is_finite() {
                return false;
            }
            transposed_activity[*column] += coefficient * rows[row_index];
        }
    }
    ...
    let mut has_general_finite_bound = false;
    for column in 0..lp_model.num_variables() {
        let lower = lp_model.basic.lb[column];
        let upper = lp_model.basic.ub[column];
        if !valid_bound_pair(lower, upper) || lower > upper {
            return false;
        }
        let has_bound_dual = !(lower.is_infinite() && upper.is_infinite())
            && !(lower == 0.0 && upper.is_infinite())
            && !(lower.is_infinite() && upper == 0.0);
        if has_bound_dual {
            has_general_finite_bound = true;
        } else if !approximately_equal_certificate(transposed_activity[column], 0.0) {
            return false;
        }
    }

    let row_rhs_product = lp_model
        .basic
        .b
        .iter()
        .zip(rows.iter())
        ...
        .sum::<f64>();
    if !row_rhs_product.is_finite() {
        return false;
    }
    has_general_finite_bound || row_rhs_product < -CERTIFICATE_TOLERANCE
}

fn resolve_lp_dual_solution_by_model<S>(
    solver: &S,
    lp_model: &LinearTriadModel,
    options: Option<&ospf_rust_core::solver::SolveOptions<'_>>,
) -> Result<Vec<f64>>
    ...
    S: Solver + Send + Sync,
{
    ...
    let dual_report = match options {
        Some(options) => solver.solve_linear_report_with_options(&dual_model, options)?,
        None => solver.solve_linear_report(&dual_model)?,
    };
    if !dual_report.is_optimal() {
        return Err(CoreError::Solver(SolverError::ContractViolation(
            "explicit dual fallback did not produce a verified optimal report".to_owned(),
        )));
    }
    let dual_solution = dual_report
        .solution
        .as_ref()
        .map(|solution| solution.values.clone())
        .ok_or_else(|| {
            CoreError::Solver(SolverError::ContractViolation(
                "dual model report has no solution vector".to_owned(),
            ))
        })?;
    let expected_rows = lp_model.num_constraints();
    ...
    if dual_solution[..expected_rows]
        .iter()
        .any(|value| !value.is_finite())
    {
        return Err(CoreError::Solver(SolverError::ContractViolation(
            "explicit dual fallback returned a non-finite multiplier".to_owned(),
        )));
    }
    ...
    S: Solver + Send + Sync,
{
    ...
    // Rebind the caller model fingerprint for the legacy facade; new paths pass the report directly.
    compatibility_report.fingerprints.model = Some(model_fingerprint);
    resolve_farkas_dual_solution_from_report(solver, lp_model, &compatibility_report)
}

pub(crate) fn resolve_farkas_dual_solution_from_report<S>(
    solver: &S,
    lp_model: &LinearTriadModel,
    report: &SolveReport<f64>,
) -> Result<Vec<f64>>
where
    S: Solver + Send + Sync,
{
    resolve_farkas_dual_solution_from_report_with_options(solver, lp_model, report, None)
}

pub(crate) fn resolve_farkas_dual_solution_from_report_with_options<S>(
    solver: &S,
    lp_model: &LinearTriadModel,
    report: &SolveReport<f64>,
    options: Option<&ospf_rust_core::solver::SolveOptions<'_>>,
...
{
    let model_fingerprint = linear_model_fingerprint(lp_model)?;
    require_infeasibility_certificate_for_model(report, &model_fingerprint)?;
    let evidence = report
        .proof
        .as_ref()
        .filter(|proof| proof.kind == ospf_rust_core::solver::ProofKind::Infeasibility)
    ...
    for candidate in native_candidates.into_iter().flatten() {
        if farkas_row_certificate_is_valid(lp_model, &candidate) {
            return Ok(candidate[..lp_model.num_constraints()].to_vec());
        }
    }
    resolve_farkas_dual_solution_by_model(solver, lp_model, options)
}
    ...
    S: Solver + Send + Sync,
{
    ...
    let farkas_report = match options {
        Some(options) => solver.solve_linear_report_with_options(&farkas_model, options)?,
        None => solver.solve_linear_report(&farkas_model)?,
    };
    if !farkas_report.is_optimal() {
        return Err(CoreError::Solver(SolverError::ContractViolation(
            "explicit Farkas fallback did not produce a verified optimal report".to_owned(),
        )));
    }
    let farkas_solution = farkas_report
        .solution
        .as_ref()
        .map(|solution| solution.values.clone())
        .ok_or_else(|| {
            CoreError::Solver(SolverError::ContractViolation(
                "Farkas dual model report has no solution vector".to_owned(),
            ))
        })?;
    let expected_rows = lp_model.num_constraints();
    ...
    if farkas_solution[..expected_rows]
        .iter()
        .any(|value| !value.is_finite())
    {
        return Err(CoreError::Solver(SolverError::ContractViolation(
            "explicit Farkas fallback returned a non-finite multiplier".to_owned(),
        )));
    }
    ...
    S: Solver + SolverExt + Send + Sync,
{
    ...
    fn solve_milp_impl(
        ...
        let report = self.solve_milp_report_impl(model, options)?;
        FeasibleSolution::try_from_report(&report)
    }

    fn solve_milp_report_impl(
        &self,
        model: &LinearTriadModel,
        options: &super::FrameworkSolveOptions,
        ...
        self.solver
            .solve_linear_report_with_options(model, &core_options)
    }
    ...
    {
        let report = self.solve_milp_report_impl(model, options)?;
        FeasibleSolution::try_from_report(&report)?.try_into_typed(options.value_conversion_policy)
    }
    ...
    fn solve_lp_impl(
        ...
        let report = self.solve_lp_report_impl(model, options)?;
        LPResult::try_from_solve_report(&report)
    }

    fn solve_lp_report_impl(
        &self,
        model: &LinearTriadModel,
        options: &super::FrameworkSolveOptions,
    ) -> Result<SolveReport<f64>> {
        ...
        let mut report = self
            .solver
            .solve_linear_report_with_options(&lp_model, &core_options)?;
        if report.is_optimal() {
            match resolve_lp_dual_solution_from_report_with_options(
                &self.solver,
                &lp_model,
                &report,
                Some(&core_options),
                ...
                Ok(dual) => {
                    if let Some(solution) = report.solution.as_mut() {
                        solution.dual_solution = Some(dual);
                    }
                }
                Err(error) => {
                    // 对偶不可验证时只能保留 incumbent，不能继续声称 exact LP。
                    // When the dual cannot be verified, retain only the incumbent and do not
                    // continue claiming an exact LP result.
                    report.solution_presence = SolutionPresence::Incumbent;
                    report.proof = None;
                    report.diagnostics.issues.push(SolveIssue::new(
                        "LpDualUnavailable",
                        format!("LP dual could not be verified: {}", error),
                    ));
                }
            }
        } else if report.has_incumbent() {
            report.diagnostics.issues.push(SolveIssue::new(
                "LpNotCertifying",
                "LP incumbent is retained, but its termination is not an exact optimality certificate",
            ));
        }
        report.validate()?;
        Ok(report)
        ...
        V: ospf_rust_core::solver::SolveValue,
    {
        self.solve_lp_impl(model, options)?
            .try_into_typed(options.value_conversion_policy)
    }
}
    ...
    S: Solver + SolverExt + Send + Sync,
{
    ...
    async fn solve_milp_report_with_options(
        ...
        self.solve_milp_report_impl(model, &options)
    }
    ...
    async fn solve_lp_report_with_options(
        ...
        self.solve_lp_report_impl(model, &options)
    }
    ...
    S: Solver + SolverExt + Send + Sync,
{
    ...
    fn solve_master_report_impl(
        &self,
        model: &LinearTriadModel,
        cuts: &[LinearCut],
    ) -> Result<SolveReport<f64>> {
        let augmented_model = apply_linear_cuts(model, cuts)?;
        self.solver.solve_linear_report(&augmented_model)
    }

    fn solve_master_report_impl_with_options(
        &self,
        model: &LinearTriadModel,
        cuts: &[LinearCut],
        ...
        self.solver
            .solve_linear_report_with_options(&augmented_model, &core_options)
    }
    ...
    fn solve_sub_impl(
        ...
        self.solve_sub_impl_with_options(model, master_solution, &FrameworkSolveOptions::default())
    }

    fn solve_sub_impl_with_options(
        &self,
        model: &LinearTriadModel,
        master_solution: &[f64],
        options: &FrameworkSolveOptions,
        ...
        let sub_report = self
            .solver
            .solve_linear_report_with_options(&fixed_subproblem, &core_options)?;
        if sub_report.has_incumbent() {
            if !sub_report.is_optimal() {
                // 保留非最优 incumbent，但禁止提取 dual 或生成 cut；外层 Benders
                // 将以实际 termination reason 停止并返回 Feasible 报告。
                // Preserve a non-optimal incumbent, but do not extract duals or cuts; the outer
                // Benders loop stops with the actual termination reason and a Feasible report.
                let feasible = FeasibleSolution::try_from_report(&sub_report)?;
                return Ok(LinearSubResult::Feasible(
                    LinearFeasibleResult::new(feasible, LinearDualSolution::default())
                        .with_report(sub_report),
                ));
            }
            let feasible = FeasibleSolution::try_from_report(&sub_report)?;
            let dual_solution = resolve_lp_dual_solution_from_report_with_options(
                &self.solver,
                &fixed_subproblem,
                &sub_report,
                Some(&core_options),
            )?;
            let mut certified_report = sub_report.clone();
            if let Some(solution) = certified_report.solution.as_mut() {
                solution.dual_solution = Some(dual_solution.clone());
            }
            let mut result = LinearFeasibleResult::new(
                feasible,
                LinearDualSolution::new(dual_solution.clone(), Vec::new()),
            )
            .with_report(certified_report);
        ...
        if sub_report.problem_status == ospf_rust_core::solver::ProblemStatus::Infeasible {
            require_infeasibility_certificate(&sub_report)?;
            let farkas_solution = resolve_farkas_dual_solution_from_report_with_options(
                &self.solver,
                &fixed_subproblem,
                &sub_report,
                Some(&core_options),
            ...
            let mut certified_report = sub_report.clone();
            if let Some(proof) = certified_report.proof.as_mut() {
                proof.evidence = Some(farkas_solution.clone());
            }
        ...
        if sub_report.problem_status == ospf_rust_core::solver::ProblemStatus::Unbounded {
            return Err(CoreError::Solver(SolverError::Unbounded));
        }

        Err(CoreError::Solver(
            solver_error_from_report_without_incumbent(&sub_report),
        ))
    }
}
    ...
    S: Solver + SolverExt + Send + Sync,
{
    ...
    async fn solve_master_report(
        ...
        self.solve_master_report_impl(model, cuts)
    }
    ...
    async fn solve_master_report_with_options(
        ...
        self.solve_master_report_impl_with_options(model, cuts, options)
    }
    ...
    async fn solve_sub_with_options(
        ...
        self.solve_sub_impl_with_options(model, master_solution, options)
    }
    ...
    S: Solver + SolverExt + Send + Sync,
{
    ...
    fn solve_master_quadratic_report_impl(
        &self,
        model: &QuadraticTetradModel,
        linear_cuts: &[LinearCut],
        quadratic_cuts: &[QuadraticCut],
        ...
        let augmented_model = apply_cuts_to_quadratic_model(model, linear_cuts, quadratic_cuts)?;
        self.solver.solve_quadratic_report(&augmented_model)
    }

    fn solve_master_quadratic_report_impl_with_options(
        &self,
        model: &QuadraticTetradModel,
        linear_cuts: &[LinearCut],
        quadratic_cuts: &[QuadraticCut],
        ...
        let core_options = options.to_core_solve_options(None);
        self.solver
            .solve_quadratic_report_with_options(&augmented_model, &core_options)
    }
    ...
    fn solve_sub_quadratic_impl(
        ...
        self.solve_sub_quadratic_impl_with_options(
            model,
            master_solution,
    ...
    fn solve_sub_quadratic_impl_with_options(
        &self,
        model: &QuadraticTetradModel,
        master_solution: &[f64],
        options: &FrameworkSolveOptions,
        ...
        let sub_report = self
            .solver
            .solve_quadratic_report_with_options(&fixed_subproblem, &core_options)?;
        if sub_report.has_incumbent() {
            if !sub_report.is_optimal() {
                // 保留非最优 incumbent，但禁止提取 dual 或生成 cut；外层 Benders
                // 将以实际 termination reason 停止并返回 Feasible 报告。
                ...
                // Benders loop stops with the actual termination reason and a Feasible report.
                let feasible = FeasibleSolution::try_from_report(&sub_report)?;
                return Ok(QuadraticSubResult::Feasible(QuadraticFeasibleResult::new(
                    LinearFeasibleResult::new(feasible, LinearDualSolution::default())
                        .with_report(sub_report),
                )));
            }
            let feasible = FeasibleSolution::try_from_report(&sub_report)?;
            let mut dual_solution = sub_report
                .solution
                .as_ref()
                .and_then(|solution| {
                    solution
                        ...
                        .clone()
                        .or_else(|| solution.dual_solution.clone())
                })
                .unwrap_or_default();
            if dual_solution.is_empty() {
                    ...
                    quadratic_model_to_linear_surrogate(&fixed_subproblem)
                {
                    let linear_report = self
                        .solver
                        .solve_linear_report_with_options(&linear_surrogate, &core_options)?;
                    dual_solution = resolve_lp_dual_solution_from_report_with_options(
                        &self.solver,
                        &linear_surrogate,
                        &linear_report,
                        Some(&core_options),
        ...
        if sub_report.problem_status == ospf_rust_core::solver::ProblemStatus::Infeasible {
            require_infeasibility_certificate(&sub_report)?;
            let mut farkas_solution = sub_report
                .solution
                .as_ref()
                .and_then(|solution| {
                    solution
                ...
                .or_else(|| {
                    sub_report
                        ...
                        .as_ref()
                        .filter(|proof| {
                            proof.kind == ospf_rust_core::solver::ProofKind::Infeasibility
                        })
                        .and_then(|proof| proof.evidence.clone())
                })
                .unwrap_or_default();
            if farkas_solution.is_empty() {
                    ...
                    quadratic_model_to_linear_surrogate(&fixed_subproblem)
                {
                    let linear_report = self
                        .solver
                        .solve_linear_report_with_options(&linear_surrogate, &core_options)?;
                    farkas_solution = resolve_farkas_dual_solution_from_report_with_options(
                        &self.solver,
                        &linear_surrogate,
                        &linear_report,
                        Some(&core_options),
    ...
    fn solve_master_linear_report_impl(
        &self,
        model: &LinearTriadModel,
        cuts: &[LinearCut],
    ) -> Result<SolveReport<f64>> {
    ...
    fn solve_sub_linear_impl(
        ...
        let fixed_variable_ids = self
            ...
            .map(|context| context.fixed_variable_ids.as_slice())
            .unwrap_or_default();
        fix_linear_subproblem_variables(&mut fixed_subproblem, master_solution, fixed_variable_ids);
        fixed_subproblem.linear_relax();

        let sub_report = self.solver.solve_linear_report(&fixed_subproblem)?;
        if sub_report.has_incumbent() {
            if !sub_report.is_optimal() {
                // 保留非最优 incumbent，但禁止提取 dual 或生成 cut；外层 Benders
                // 将以实际 termination reason 停止并返回 Feasible 报告。
            ...
            let feasible = FeasibleSolution::try_from_report(&sub_report)?;
            let dual_solution =
                resolve_lp_dual_solution_from_report(&self.solver, &fixed_subproblem, &sub_report)?;
            let mut certified_report = sub_report.clone();
            if let Some(solution) = certified_report.solution.as_mut() {
                solution.dual_solution = Some(dual_solution.clone());
            }
        ...
        if sub_report.problem_status == ospf_rust_core::solver::ProblemStatus::Infeasible {
            require_infeasibility_certificate(&sub_report)?;
            let farkas_solution = resolve_farkas_dual_solution_from_report(
                &self.solver,
                &fixed_subproblem,
                &sub_report,
            )?;
    ...
    S: Solver + SolverExt + Send + Sync,
{
    ...
    async fn solve_master_report(
        ...
        self.solve_master_linear_report_impl(model, cuts)
    }
    ...
    S: Solver + SolverExt + Send + Sync,
{
    ...
    async fn solve_master_quadratic_report(
        ...
        self.solve_master_quadratic_report_impl(model, linear_cuts, quadratic_cuts)
    }
    ...
    async fn solve_master_quadratic_report_with_options(
        ...
        self.solve_master_quadratic_report_impl_with_options(
            model,
            linear_cuts,
            quadratic_cuts,
            options,
    ...
    async fn solve_sub_quadratic_with_options(
        ...
        self.solve_sub_quadratic_impl_with_options(model, master_solution, options)
    }
...
mod tests {
    ...
    use ospf_rust_core::solver::{
        LinearSolver, QuadraticSolver, SolveReport, SolverCapability, SolverInfo, SolverProvenance,
        SolverStatus, TerminationReason, linear_model_fingerprint, quadratic_model_fingerprint,
        solver_output_to_report_with_provenance, verify_native_terminal_proof,
    };
    use ospf_rust_core::token::Token;
    use ospf_rust_core::variable::{ContinuousVariableItem, UContinuousVariableItem};

    #[test]
    fn report_without_incumbent_keeps_terminal_and_backend_error_boundaries() {
        let time_limit = SolveReport::builder(
            ospf_rust_core::solver::ProblemStatus::Unknown,
            ospf_rust_core::solver::TerminationReason::TimeLimit,
        )
        .build()
        .expect("time-limit report should be valid");
        assert!(matches!(
            solver_error_from_report_without_incumbent(&time_limit),
            SolverError::Timeout(_)
        ));

        let cancelled = SolveReport::builder(
            ospf_rust_core::solver::ProblemStatus::Unknown,
            ospf_rust_core::solver::TerminationReason::Cancelled,
        )
        .build()
        ...
        assert!(matches!(
            solver_error_from_report_without_incumbent(&cancelled),
            SolverError::Cancelled(_)
        ));

        let backend_failure = SolveReport::builder(
            ospf_rust_core::solver::ProblemStatus::Unknown,
            ospf_rust_core::solver::TerminationReason::BackendFailure,
        )
        .build()
        .expect("backend-failure report should be valid");
        assert!(matches!(
            solver_error_from_report_without_incumbent(&backend_failure),
            SolverError::SolveFailed(_)
        ));
    }
    ...
    #[derive(Debug)]
    struct VerifiedInfeasibleColumnGenerationSolver;
    ...
    fn quadratic_subproblem_fixes_only_declared_shared_variables_by_id() {
        ...
        assert!(
            model.basic.linear.lb[0].is_infinite() && model.basic.linear.lb[0].is_sign_negative()
        );
        assert!(
            model.basic.linear.ub[0].is_infinite() && model.basic.linear.ub[0].is_sign_positive()
        );
    ...
    fn solver_output_to_feasible_preserves_time_limit_diagnostic_message() {
        ...
        assert!(matches!(err, CoreError::Solver(SolverError::Timeout(_))));
    }
    ...
    impl SolverInfo for VerifiedInfeasibleColumnGenerationSolver {
        fn name(&self) -> &str {
            "verified_infeasible_column_generation_solver"
        }
    ...
    impl LinearSolver for VerifiedInfeasibleColumnGenerationSolver {
        fn solve_linear(&self, _model: &LinearTriadModel) -> Result<SolverOutput> {
            Ok(SolverOutput::new(SolverStatus::Infeasible))
        }

        fn solve_linear_report(&self, model: &LinearTriadModel) -> Result<SolveReport<f64>> {
            verified_linear_report(model, SolverOutput::new(SolverStatus::Infeasible))
        }
    }

    impl QuadraticSolver for VerifiedInfeasibleColumnGenerationSolver {
        fn solve_quadratic(&self, _model: &QuadraticTetradModel) -> Result<SolverOutput> {
            Ok(SolverOutput::new(SolverStatus::Infeasible))
        }

        fn solve_quadratic_report(&self, model: &QuadraticTetradModel) -> Result<SolveReport<f64>> {
            verified_quadratic_report(model, SolverOutput::new(SolverStatus::Infeasible))
        }
    }
    ...
    fn build_infeasible_free_variable_model() -> LinearTriadModel {
        let mut basic = BasicLinearTriadModel::new("infeasible_free_variable");
        basic.add_variable_with_bounds(
            Token::from_generic(ContinuousVariableItem::auto("x"), 0),
            f64::NEG_INFINITY,
            f64::INFINITY,
            ospf_rust_core::variable::VariableType::Continuous,
        );
        let mut upper = SparseVector::new();
        upper.add(0, 1.0);
        basic.add_constraint(upper, -1.0);
    ...
    #[test]
    fn column_generation_lp_report_preserves_verified_infeasibility() {
        let model = build_infeasible_free_variable_model();
        let adapter = CoreColumnGenerationAdapter::new(
            "verified_infeasible_column_generation",
            VerifiedInfeasibleColumnGenerationSolver,
        );

        let report = adapter
            .solve_lp_report_impl(&model, &FrameworkSolveOptions::default())
            .expect("report-first LP entry should preserve verified infeasibility");
        assert_eq!(
            report.problem_status,
            ospf_rust_core::solver::ProblemStatus::Infeasible
        );
        assert_eq!(
        ...
        assert!(
            require_infeasibility_certificate_for_model(
                &report,
                &linear_model_fingerprint(&model).expect("model fingerprint should be available")
            )
            .is_ok(),
            "verified infeasible report must remain consumable as a certificate"
        );

        let legacy_result = adapter.solve_lp_impl(&model, &FrameworkSolveOptions::default());
        let error = legacy_result.expect_err(
            "legacy LPResult must reject infeasibility instead of erasing the report contract",
        );
        assert!(matches!(
            error,
            CoreError::Solver(SolverError::ContractViolation(message))
                if message.contains("LP report is not certifying")
        ));
    }

    fn verified_linear_report(
        model: &LinearTriadModel,
        output: SolverOutput,
    ) -> Result<SolveReport<f64>> {
        let native_status = output.status;
        let mut report = solver_output_to_report_with_provenance(
            output,
            SolverProvenance {
                solver_id: "test/native".to_owned(),
    ...
    fn verified_quadratic_report(
        ...
        let mut report = solver_output_to_report_with_provenance(
            output,
            SolverProvenance {
                solver_id: "test/native-quadratic".to_owned(),
                backend_name: "test".to_owned(),
                ..SolverProvenance::default()
            },
        )?;
    ...
    impl LinearSolver for DualFallbackSolver {
        fn solve_linear(&self, model: &LinearTriadModel) -> Result<SolverOutput> {
            if model.basic.name.ends_with("_farkas_dual") {
                return Ok(SolverOutput::optimal(0.0, vec![0.5, 0.5]));
            }
        ...
        fn solve_linear_report(&self, model: &LinearTriadModel) -> Result<SolveReport<f64>> {
            verified_linear_report(model, self.solve_linear(model)?)
        }
    }

    impl QuadraticSolver for DualFallbackSolver {
        fn solve_quadratic(&self, model: &QuadraticTetradModel) -> Result<SolverOutput> {
            Ok(SolverOutput::optimal(0.0, vec![0.0; model.num_variables()]))
        }

        fn solve_quadratic_report(&self, model: &QuadraticTetradModel) -> Result<SolveReport<f64>> {
            verified_quadratic_report(model, self.solve_quadratic(model)?)
        }
    }

    #[derive(Debug)]
    struct DualUnavailableSolver;

    impl SolverInfo for DualUnavailableSolver {
        fn name(&self) -> &str {
            "dual_unavailable_solver"
        }
    ...
    impl LinearSolver for DualUnavailableSolver {
        fn solve_linear(&self, model: &LinearTriadModel) -> Result<SolverOutput> {
            if model.basic.name.ends_with("_dual") {
                return Err(CoreError::solver_backend(
                    "explicit dual backend is unavailable",
                ));
            }
            Ok(SolverOutput::optimal(2.0, vec![2.0]))
        }
        ...
        fn solve_linear_report_with_options(
            &self,
            model: &LinearTriadModel,
            _options: &ospf_rust_core::solver::SolveOptions<'_>,
        ) -> Result<SolveReport<f64>> {
            self.solve_linear_report(model)
        }
    }

    impl QuadraticSolver for DualUnavailableSolver {
        fn solve_quadratic(&self, model: &QuadraticTetradModel) -> Result<SolverOutput> {
            Ok(SolverOutput::optimal(0.0, vec![0.0; model.num_variables()]))
        }
    ...
    #[derive(Debug)]
    struct NonOptimalSubproblemSolver;

    impl SolverInfo for NonOptimalSubproblemSolver {
        fn name(&self) -> &str {
            "non_optimal_subproblem_solver"
        }
    ...
    impl LinearSolver for NonOptimalSubproblemSolver {
        fn solve_linear(&self, _model: &LinearTriadModel) -> Result<SolverOutput> {
            Ok(SolverOutput::new(SolverStatus::TimeLimit)
                .with_objective(2.0)
                .with_solution(vec![2.0]))
        }
    ...
    impl QuadraticSolver for NonOptimalSubproblemSolver {
        fn solve_quadratic(&self, _model: &QuadraticTetradModel) -> Result<SolverOutput> {
            Ok(SolverOutput::new(SolverStatus::TimeLimit)
                .with_objective(3.0)
                .with_solution(vec![2.0]))
    ...
    #[test]
    fn benders_preserves_nonoptimal_subproblem_incumbents_without_cuts() {
        let linear_adapter =
            CoreLinearBendersAdapter::new("non_optimal_linear", NonOptimalSubproblemSolver);
        let linear_result = linear_adapter
            .solve_sub_impl(&build_single_row_lp_model(), &[2.0])
            .expect("non-optimal linear subproblem should remain a feasible result");
        match linear_result {
            LinearSubResult::Feasible(result) => {
                assert_eq!(result.result.obj, 2.0);
                assert!(result.dual_solution.constraints.is_empty());
                assert!(result.cuts.is_none());
                assert_eq!(
                    result
                        .report
                        .as_ref()
                ...
                assert!(
                    !result
                        .report
                        .as_ref()
                        .expect("subproblem report should be retained")
                        .is_optimal()
                );
            }
            LinearSubResult::Infeasible(_) => {
                panic!("non-optimal linear subproblem must not become infeasible")
            }
        }

        let quadratic_adapter =
            CoreQuadraticBendersAdapter::new("non_optimal_quadratic", NonOptimalSubproblemSolver);
        let quadratic_result = quadratic_adapter
            .solve_sub_quadratic_impl(&build_quadratic_subproblem_model(), &[2.0])
            .expect("non-optimal quadratic subproblem should remain a feasible result");
        match quadratic_result {
            QuadraticSubResult::Feasible(result) => {
                assert_eq!(result.linear.result.obj, 3.0);
                assert!(result.linear.dual_solution.constraints.is_empty());
                assert!(result.linear.cuts.is_none());
                assert!(result.quadratic_cuts.is_none());
                assert_eq!(
                    result
                        .linear
                        .report
            ...
            QuadraticSubResult::Infeasible(_) => {
                panic!("non-optimal quadratic subproblem must not become infeasible")
            }
        }
    }
    ...
    fn resolve_lp_dual_solution_keeps_consistent_native_dual() {
        ...
        let report =
            verified_linear_report(&model, output).expect("native LP report should be verified");
        let dual = resolve_lp_dual_solution_from_report(&DualFallbackSolver, &model, &report)
    ...
    #[test]
    fn lp_report_downgrades_when_dual_cannot_be_verified() {
        let model = build_single_row_lp_model();
        let adapter = CoreColumnGenerationAdapter::new("dual_unavailable", DualUnavailableSolver);

        let report = adapter
            .solve_lp_report_impl(&model, &FrameworkSolveOptions::default())
            .expect("an incumbent should still be returned when the dual is unavailable");
        ...
        assert_eq!(
            ...
            "a missing dual must remove the exact LP solution presence"
        );
        ...
        assert!(
            report
                ...
                .issues
                .iter()
                .any(|issue| issue.code == "LpDualUnavailable")
        );
        report
            .validate()
            .expect("downgraded report should validate");
    }

    #[test]
    fn dual_fallback_honors_cancellation_options() {
        let model = build_single_row_lp_model();
        let output = SolverOutput::optimal(2.0, vec![2.0]);
        let report =
            verified_linear_report(&model, output).expect("native LP report should be verified");
        let handle = ospf_rust_core::solver::SolveHandle::new();
        assert!(handle.cancel(ospf_rust_core::solver::CancellationOrigin::User));
        let options =
            ospf_rust_core::solver::SolveOptions::new().with_cancellation_handle(Some(&handle));

        let result = resolve_lp_dual_solution_from_report_with_options(
            &DualFallbackSolver,
            &model,
            &report,
            Some(&options),
        ...
        assert!(result.is_err(), "cancelled fallback must not return a dual");
    }
    ...
    fn resolve_farkas_dual_solution_keeps_solver_provided_certificate() {
        ...
        output.dual_solution = Some(vec![3.0, 3.0]);

        let report = verified_linear_report(&model, output)
            .expect("native Farkas report should be verified");
        let farkas = resolve_farkas_dual_solution_from_report(&DualFallbackSolver, &model, &report)
            .expect("solver-provided Farkas certificate should be accepted");
        assert_eq!(farkas, vec![3.0, 3.0]);
    }
    ...
    fn resolve_farkas_dual_solution_truncates_extra_native_entries() {
        let model = build_infeasible_free_variable_model();
        let mut output = SolverOutput::new(SolverStatus::Infeasible);
        output.dual_solution = Some(vec![3.0, 3.0, 4.0]);
    ...
    fn resolve_farkas_dual_solution_falls_back_to_explicit_model_when_missing() {
        ...
        let report = verified_linear_report(&model, output)
            .expect("native infeasibility report should be verified");
        let farkas = resolve_farkas_dual_solution_from_report(&DualFallbackSolver, &model, &report)
            .expect("missing Farkas certificate should fall back to explicit model");
        assert_eq!(farkas, vec![0.5, 0.5]);
    }
