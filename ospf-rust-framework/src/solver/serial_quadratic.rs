...
use super::parallel_quadratic::QuadraticSolver;
use super::{FeasibleSolution, SolveOptions};
...
pub struct SerialCombinatorialQuadraticSolver {
    /// 求解器列表 / Solver list
...
impl SerialCombinatorialQuadraticSolver {
    /// 创建新的串行组合二次求解器 / Create new serial combinatorial quadratic solver
...
mod tests {
    use super::*;
    #[cfg(not(feature = "async"))]
    use crate::solver::parallel_quadratic::QuadraticMetaModelSolverExt;
    #[cfg(not(feature = "async"))]
    ...
    fn test_serial_stops_on_infeasible_error() {
        ...
        impl QuadraticSolver for StopMockSolver {
            fn name(&self) -> &str {
                &self.name
            }
        ...
        let solver = SerialCombinatorialQuadraticSolver::new(solvers);
        let model = QuadraticTetradModel::new("serial_stop_q");
    ...
    fn test_serial_solver_supports_meta_model_shortcut() {
        ...
        let solver = SerialCombinatorialQuadraticSolver::new(solvers);
        let mut meta_model = MetaModel::<f64>::new("serial_quadratic_meta_shortcut");
        let x = ContinuousVariableItem::auto("serial_quadratic_meta_shortcut_x");
        ...
        meta_model
            .add_linear_constraint(
                ...
                1.0,
                "serial_quadratic_meta_shortcut_c",
            )
            .unwrap();

        let result = solver
            .solve_meta(&meta_model)
            .expect("serial quadratic solver meta shortcut should succeed");
    ...
    fn test_serial_solver_meta_shortcut_rejects_non_finite_value() {
        ...
        let solver = SerialCombinatorialQuadraticSolver::new(solvers);
        let mut meta_model = MetaModel::<f64>::new("serial_quadratic_meta_non_finite");
        let x = ContinuousVariableItem::auto("serial_quadratic_meta_non_finite_x");
        let x_index = meta_model.register_variable(x).unwrap();
        meta_model
            .add_linear_constraint(
                &[(x_index, f64::NAN)],
                ConstraintRelation::LessEqual,
                1.0,
                "serial_quadratic_meta_non_finite_c",
            )
            .unwrap();
