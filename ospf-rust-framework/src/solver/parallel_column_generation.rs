...
use super::{
    ColumnGenerationSolver, FeasibleSolution, LPResult, ObjectiveCategory,
    ParallelCombinatorialMode, SolveOptions,
};
...
impl ColumnGenerationSolver for ParallelCombinatorialColumnGenerationSolver {
        ...
        match self.mode {
            ParallelCombinatorialMode::First => {
                ...
                for (solver_index, solver) in self.solvers.iter().enumerate() {
                    ...
                    tasks.spawn(
                        async move { solver.solve_milp_with_options(&model, options).await },
                    );
                }
    ...
    async fn solve_lp_with_options(
        ...
        match self.mode {
            ParallelCombinatorialMode::First => {
                let mut tasks: JoinSet<Result<LPResult>> = JoinSet::new();
                ...
                for (solver_index, solver) in self.solvers.iter().enumerate() {
                    ...
                    let options = options
                        ...
                        .with_registration_callback(registration_callback)
                        .with_solving_callback(solving_callback);
                    tasks.spawn(async move { solver.solve_lp_with_options(&model, options).await });
                }
                ...
                let handles: Vec<_> = self
                    ...
                    .map(|(solver_index, solver)| {
                        ...
                        let options = options
                            ...
                            .with_registration_callback(registration_callback)
                            .with_solving_callback(solving_callback);
                        thread::spawn(move || solver.solve_milp_with_options(&model, options))
                    })
                    .collect();
                ...
                let handles: Vec<_> = self
                    ...
                    .map(|(solver_index, solver)| {
                        ...
                        let options = options
                            ...
                            .with_registration_callback(registration_callback)
                            .with_solving_callback(solving_callback);
                        thread::spawn(move || solver.solve_lp_with_options(&model, options))
                    })
                    .collect();
...
mod tests {
    ...
    fn test_best_mode_respects_maximum_objective_category() {
        ...
        let milp_result = solver
            .solve_milp_with_options(&model, SolveOptions::new().with_name("max_case"))
            .expect("best mode should return one feasible solution");
        assert!((milp_result.obj - 2.0).abs() <= 1e-9);

        let lp_result = solver
            .solve_lp_with_options(&model, SolveOptions::new().with_name("max_case"))
            .expect("best mode should return one feasible solution");
