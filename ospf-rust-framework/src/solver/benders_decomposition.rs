...
use super::{
    ...
    LinearDualSolution, SolveOptions,
};
...
pub trait LinearBendersDecompositionSolver: Send + Sync {
    ...
    {
        self.solve_meta_with_options(master_meta_model, sub_meta_model, SolveOptions::default())
    }
    ...
    async fn solve(
        ...
        for iter in 0..max_iterations {
            ...
            let master_solution = match &master_result.solution {
                Some(s) => s.clone(),
                None => return Err(CoreError::Solver(SolverError::Infeasible)),
            };

            let master_obj = master_result.objective_value.unwrap_or(0.0);
...
mod tests {
    ...
    use ospf_rust_core::model::intermediate::{
        BasicQuadraticTetradModel, LinearTriadModel, QuadraticTetradModel,
    };
    ...
    fn test_linear_benders_cut_stall_window_stops_early() {
        ...
        assert_eq!(runtime_metrics.stop_reason, Some(BendersStopReason::CutStall));
    ...
    async fn test_linear_benders_meta_model_with_options_returns_send_future() {
        ...
        let options = SolveOptions::new().with_iterations(5, 1e-9);
